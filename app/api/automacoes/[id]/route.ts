...
export async function POST(request: NextRequest, { params }: RouteContext<"/api/automacoes/[id]">) {
  ...
  if (body.action === "update-automation") ({ error } = await supabase.from("automacoes").update({ nome: body.nome, gatilho: body.gatilho, status: body.status }).eq("id", id).eq("user_id", user.id));
  if (body.action === "create-stage") ({ error } = await supabase.from("automacao_etapas").insert({ automacao_id: id, nome: body.stage.nome, tipo: body.stage.tipo, config: body.stage.config ?? {}, position_x: 0, position_y: body.stage.position_y }));
